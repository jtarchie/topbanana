---
{
  ...
  "setup_notes": "Two pages ship with this site. index.html is the upload page — print its URL as a QR code and put it on tables or signage so guests can scan and upload.\n\ndisplay.html is the big-screen wall — open it full-screen on the projector or TV. It checks for newly-approved photos every few seconds, so nothing needs reloading during the event.\n\nEvery uploaded photo waits for you: open the moderation queue (the Photo wall card on this page) and tap Approve to send a photo to the display, or Reject to discard it."
}
---
