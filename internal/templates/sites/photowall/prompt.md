...
- index.html is the upload page guests reach by scanning a QR code. It has a clear heading, a one-line invitation to scan and share a photo, and an HTML form that posts a single photo:
  ...
  Style the file input as a large tap-to-choose photo area (an icon + a short prompt), not a bare `<input type="file">` and never a daisyUI text `input` — the skeleton's `.photo-drop` control is the reference. Keep the real `<input type="file" name="photo">` inside it.
