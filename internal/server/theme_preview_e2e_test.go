...
//
// The workspace fires postMessage({type:'buildabear:settheme', theme}) at the
...
func TestThemePreviewListener_AlwaysInjected(t *testing.T) {
	...
	anonBody := get(t, nil)
	if !strings.Contains(anonBody, "buildabear:settheme") {
	...
	if !strings.Contains(authedBody, "buildabear:settheme") {
