...
// adds something we'd want.
const polishPrompt = `You are polishing an already-built site. The site lints clean and works — your job is the meticulous final pass that catches small details separating good from great. Read files before editing them and edit in place; do not rewrite pages from scratch or delete content the user did not ask you to remove. Stop and say "done" when no further worthwhile improvements remain; do not invent content the user did not request.
