...
// TestAgentTools_ListFilesShowsEditableAssets pins the listing to the editable
// set. A site authored over MCP can carry its own stylesheet, and while
// list_files hid it the agent had no way to learn that the file deciding how
// the page renders even existed. Binary uploads, the generated app.css and the
// sidecar stay hidden — the agent cannot write any of them.
func TestAgentTools_ListFilesShowsEditableAssets(t *testing.T) {
	tools, s, _ := newToolSet(t)
	mustStoreWrite(t, s, "toolslug", "index.html", "<p>hi</p>", "text/html; charset=utf-8", nil)
	mustStoreWrite(t, s, "toolslug", "site.css", ".shot{width:84px}", "text/css; charset=utf-8", nil)
	mustStoreWrite(t, s, "toolslug", "assets/pic.png", "png-bytes", "image/png", nil)
	mustStoreWrite(t, s, "toolslug", "app.css", ".generated{}", "text/css; charset=utf-8", nil)
	mustStoreWrite(t, s, "toolslug", ".topbanana.json", "{}", "application/json", nil)

	res := invokeTool(t, tools["list_files"], nil)
	files := fmt.Sprint(res["files"])
	for _, want := range []string{"index.html", "site.css"} {
		if !strings.Contains(files, want) {
			t.Errorf("list_files = %v, missing %q", files, want)
		}
	}
	for _, unwanted := range []string{"assets/pic.png", "app.css", ".topbanana.json"} {
		if strings.Contains(files, unwanted) {
			t.Errorf("list_files = %v, must not list %q", files, unwanted)
		}
	}
}

// TestAgentTools_EditsSiteStylesheet is the regression for the incident this
// widening exists to prevent: asked to make images bigger, the agent edited the
// width attributes on <img> twice while an unlayered `.shot{width:84px}` rule
// in the site's own stylesheet kept winning the cascade. It could not read that
// file, let alone fix it, and reported success both times.
func TestAgentTools_EditsSiteStylesheet(t *testing.T) {
	tools, s, _ := newToolSet(t)
	mustStoreWrite(t, s, "toolslug", "site.css", ".tool .shot{width:84px;height:84px}", "text/css; charset=utf-8", nil)

	res := invokeTool(t, tools["read_file"], map[string]any{"path": "site.css"})
	if !strings.Contains(resStr(res, "content"), "width:84px") {
		t.Fatalf("read_file could not read the site stylesheet: %v", res)
	}

	res = invokeTool(t, tools["edit_file"], map[string]any{
		"path":     "site.css",
		"old_text": "width:84px;height:84px",
		"new_text": "width:min(42%,220px);height:auto",
	})
	if !resBool(res, "ok") {
		t.Fatalf("edit_file on the site stylesheet: %v", res)
	}

	obj, err := s.Read(t.Context(), "toolslug", "site.css")
	if err != nil {
		t.Fatalf("read back: %v", err)
	}
	if !strings.Contains(obj.Content, "width:min(42%,220px)") {
		t.Errorf("stylesheet not updated: %q", obj.Content)
	}
	if obj.ContentType != "text/css; charset=utf-8" {
		t.Errorf("ContentType = %q, want the stylesheet type preserved", obj.ContentType)
	}
}

// TestAgentTools_EditGateRefusesManagedPaths keeps the widened edit surface
// from reaching anything the platform owns. app.css is regenerated by every
// build, functions/ has its own tools, and deleting or blanking the sidecar
// orphans the site.
func TestAgentTools_EditGateRefusesManagedPaths(t *testing.T) {
	tools, s, _ := newToolSet(t)
	mustStoreWrite(t, s, "toolslug", "app.css", ".generated{}", "text/css; charset=utf-8", nil)
	mustStoreWrite(t, s, "toolslug", "functions/submit.js", "module.exports=function(){}", "text/javascript", nil)

	for _, path := range []string{"app.css", "functions/submit.js", ".topbanana.json", "_state/data.json"} {
		res := invokeTool(t, tools["edit_file"], map[string]any{
			"path": path, "old_text": "a", "new_text": "b",
		})
		if resStr(res, "error") == "" {
			t.Errorf("edit_file(%q) was allowed; want a path error", path)
		}
	}
}

// TestAgentTools_WriteFileStaysHTMLOnly holds the deliberate asymmetry: the
// agent may rewrite a stylesheet the site already has, but may not author a new
// one. A second competing stylesheet on a site that already has a design system
// is the confusion this whole change exists to remove.
func TestAgentTools_WriteFileStaysHTMLOnly(t *testing.T) {
	tools, _, _ := newToolSet(t)
	res := invokeTool(t, tools["write_file"], map[string]any{
		"path": "extra.css", "content": ".x{}",
	})
	if resStr(res, "error") == "" {
		t.Errorf("write_file created a new stylesheet; want a path error: %v", res)
	}
}
