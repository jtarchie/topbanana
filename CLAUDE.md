...
**Gotchas:**
- daisyUI v5 components **are** content-purged in this pipeline (a site using no daisyUI classes compiles to ~52 KB that is almost entirely `@layer base` theme tokens, with `.card`/`.navbar`/`.hero` absent). `@source not <daisyui dir>` still matters for the utility layer.
